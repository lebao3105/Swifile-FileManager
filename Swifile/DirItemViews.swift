...
struct DirListItemActions: View {
	...
	@State private var hasDebExt: Bool = false
	@State private var showDebInst: Bool = false
	@State private var installTest: (output: String, status: Int) = ("", 0)
	
	init(item: ContentItem, isPresented: Binding<Bool>, contents: Binding<[ContentItem]>) {
		self._item = State(initialValue: item)
		self._isPresented = isPresented
		self._contents = contents
		if (item.url.pathExtension == "deb") { hasDebExt = true }
	}
	...
	var body: some View {
		NavigationView {
			VStack {
				List {
					if (hasDebExt) {
						Button {
							installTest = runCommand("/usr/bin/apt", ["install", "-f", "--allow-unauthenticated", "-y", item.realPath], 0)
							showDebInst.toggle()
						} label: {
							Label("Install", systemImage: "display.and.arrow.down")
						}
					}
		...
		.sheet(isPresented: $showDebInst) {
			LogView(text: installTest.output + "\nCommand exited with code " + String(installTest.status))
		}
	}
}
